...
//3.檢測進入頁面錯誤  
  ...
  await expect(page.getByRole('dialog', { name: '進入進貨單時錯誤' })).not.toBeVisible();
    ...
    await page.getByTestId('dialog-search-input-DRPPO-CUS_NO-1-value1').waitFor({ state: 'visible', timeout: 15000 });
    ...
    await page.waitForLoadState('networkidle');
    await waitAndClick(page.getByTestId('DRPPO-gridOptions-B-column_0-row_0-btn'), 30000);
