...
//3.1 表頭重要欄位檢測
    await expect(page.getByTestId('DRPPO-H-CUS_NO')).toHaveValue('B2B 有限公司', { timeout: 30000 });
//3.2 檢測客戶扣稅類別與立帳方式 ---有些元素不是 input，改用包含或文本檢查
    ...
    await page.getByTestId('DRPPO-TF_POS-B-QTY-row_0-input').fill('100.00');
    ...
    // 移除千分符，確保欄位接受
    await page.getByTestId('DRPPO-TF_POS-B-UP-row_0-input').fill('8888.00');
    ...
    // 修改採購單======================================= 新增訂金與 TQ 發票 55================================================
    await waitAndClick(page.getByTestId('DRPPO-radio-button-5'));
    await waitAndClick(page.getByTestId('DRPPO-invsa_tab6').getByTestId('DRPPO-H-AMTN_BC'));
    await page.getByTestId('DRPPO-invsa_tab6').getByTestId('DRPPO-H-AMTN_BC').fill('100000');
    await page.getByTestId('DRPPO-invsa_tab6').getByTestId('DRPPO-H-AMTN_BC').press('Enter');

    await waitAndClick(page.getByTestId('DRPPO-invsa_tab6').getByTestId('DRPPO-H-CACC_NO'));
    await waitAndClick(page.getByTestId('DRPPO-invsa_tab6').getByTestId('DRPPO-H-CACC_NO-icon-svg'));
    await page.getByTestId('dialog-search-input-DRPPO-BACC_NO-1-value1').waitFor({ state: 'visible', timeout: 30000 });
    await page.getByTestId('dialog-search-input-DRPPO-BACC_NO-1-value1').fill('CA01');
    await page.getByTestId('dialog-search-input-DRPPO-BACC_NO-1-value1').press('Enter');
    await waitAndClick(page.getByTestId('dialog-DRPPO-Search-btn'));
    await waitAndClick(page.getByTestId('DRPPO-gridOptions-B-column_0-row_0-btn'));
