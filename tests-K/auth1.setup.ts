...
const authFile = path.join(__dirname, '../tests-K/playwright/.auth/user-k.json');

setup('authenticate-k', async ({ page }) => {
  ...
  // 填入使用者名稱
  await page.getByTestId('default-username').fill('ADMIN');
  await page.getByTestId('default-username').press('Enter');
  await page.waitForLoadState('networkidle');
  
  // 填入密碼
  await page.locator('#divUser').getByRole('textbox').fill('ADMIN');
