...
//3.2 檢測客戶扣稅類別與立帳方式 ---有些元素不是 input，改用包含或文本檢查
  ...
  // 離開銷貨開票處理頁面
    ...
    //************************************************LONO704-銷貨單發票INVXX05修改-統一編號/未稅/稅金/含稅 ********************************************/
      ...
      await page.getByTestId('INVLZ-save-btn').click();
      await expect(page.getByText('存檔成功')).toBeVisible();
      await page.getByTestId('dialog-INVLZ-H-TAX').press('Enter');
    ...
    // 離開銷貨開票處理頁面
    await waitAndClick(page.getByTestId('INVLZ-exit2-btn'));
    await waitAndClick(page.getByTestId('dialog-INVLZ-否(N)-btn'));
    ...
    await page.waitForTimeout(500);
      ...
      //await waitAndClick(page.getByTestId('dialog-DRPSB-否(N)-btn'));
    ...
    await page.waitForTimeout(500);
      ...
      // 離開銷貨單頁面
      await page.getByTestId('DRPSA-exit2-btn').click();    
      //await waitAndClick(page.getByTestId('dialog-DRPSA-否(N)-btn'));
