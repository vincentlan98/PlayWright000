...
// 2.認證user.json 指向存放在tests/playwright/.auth/下;await page.goto('http://localhost/SunFusion/#/mon/monbx');
// 3.刪除初次開啟點擊新增按鍵;
//4A.表頭各欄位檢測含搜尋欄位(報銷單);
...
//3.檢測進入頁面錯誤  
  ...
  //await waitAndClick(page.getByTestId('MONBX-add-btn')); //預設進入不需按新增
  ...
  await waitAndClick(page.getByTestId('CASN-resize').getByTestId('dialog-MONBX-關閉-btn'));
  ...
  await page.waitForLoadState('networkidle');
  await expect(page.getByRole('dialog', { name: '進入報銷單表頭幣別設定的外幣代號時錯誤' })).not.toBeVisible();
  await waitAndClick(page.getByTestId('CUR_ID-resize').locator('div').filter({ hasText: /^編輯關閉$/ }).first());
  await waitAndClick(page.getByTestId('CUR_ID-resize').getByTestId('dialog-MONBX-關閉-btn'));
  await page.waitForLoadState('networkidle');
  ...
  //4A06.檢測表頭請款人進入頁面錯誤  
  await page.waitForLoadState('networkidle');
  await expect(page.getByRole('dialog', { name: '進入報銷單表頭請款人時錯誤' })).not.toBeVisible();
  await waitAndClick(page.getByTestId('MONBX-gridOptions-B-column_0-row_0-btn'));
...
//第二列增加輸入-先訂位置
  ...
  await waitAndClick(page.getByTestId('MONBX-TF_BX-B-TAX_ID-row_1-select-option-3-text'));
...
//第三列增加輸入-先訂位置
  ...
  await waitAndClick(page.getByTestId('MONBX-TF_BX-B-TAX_ID-row_2-select-option-3-text'));
...
//***************************************修改報銷單-2-修改表身第一列金額與發票***********************************************
  ...
  //****************************************************刪除銷貨單B01
    ...
    await expect(page.getByText('刪除成功')).toBeVisible({ timeout: 3000 });  
    //await page.waitForTimeout(5000);

  // 關閉報銷單頁面-不存檔離開
  await page.waitForLoadState('networkidle');
  await waitAndClick(page.getByTestId('MONBX-exit2-btn'));
  await waitAndClick(page.getByTestId('MONBX-詢問-dialog').locator('header'));
  await waitAndClick(page.getByTestId('dialog-MONBX-否(N)-btn'));
