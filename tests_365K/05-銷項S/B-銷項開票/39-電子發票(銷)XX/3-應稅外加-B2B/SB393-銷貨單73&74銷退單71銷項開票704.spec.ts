...
//SA73->SB71->SA74->LO704->XX05作廢&XX06---20260122版
...
//20260119 - 因為第二行的本次數量欄位修改不復見, for some reason, the Enter key press was not working as expected here, so added a click on another element to trigger the change
  ...
  //等待後端處理與畫面更新--需時較久
  await page.waitForLoadState('networkidle');
  await expect(page.getByText('存檔成功')).toBeVisible({ timeout: 60000 });
  await page.waitForLoadState('networkidle');
...
*/
  ...
  //3.檢測進入頁面錯誤  
      ...
      await page.waitForLoadState('networkidle');      
      await waitAndClick(page.getByTestId('INVLZ-TF_LZ-B-UP_ZG-row_1-cell-wrapper')); //20260206 先點擊本次單價欄位觸發後端帶入資料,再修改本次單價
      await page.waitForLoadState('networkidle');
  ...
  //3.檢測進入頁面錯誤  
    ...
    await page.waitForTimeout(500);
      ...
      // 離開銷貨單頁面
      await page.waitForLoadState('networkidle');
      //await waitAndClick(page.getByTestId('DRPSA-exit2-btn'));  //20260202 已經在刪除銷貨單SANO73時離開頁面,這裡不再重複離開一次  
